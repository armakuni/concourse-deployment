...
def set_pipeline(path, target, pipeline_name, pipeline_config_path, pipeline_vars_paths, pipeline_onepassword_key):
  ...
  os.system("export UUID=test1")#$(op get item \"" + pipeline_onepassword_key + "\" --session=$OP_SESSION_armakuni | jq '.uuid')")
  os.system("export VAULTUUID=test2")#$(op get item \"" + pipeline_onepassword_key + "\" --session=$OP_SESSION_armakuni | jq '.vaultUuid')")
  os.system("echo $UUID")
